...
stacked_like_plots <- plotl1 / plotl2 /plotl3 / plotl4 / plotl5
stacked_like_plots
```

```{r, Match Distribution Across Owners}
dbr <- d[Owner == "Brian"]
plotm1 <- ggplot(dbr, aes(x = Date, y = Matches, colors = Date)) +
  geom_line(color = "blue", size = 5) +
  labs(title = "Likes over Time (Brian)", x = "Date", y = "Matches")

der <- d[Owner == "Erin Smith"]
plotm2 <- ggplot(der, aes(x = Date, y = Matches, colors = Date)) +
  geom_line(color = "blue", size = 5) +
  labs(title = "Likes over Time (Erin)", x = "Date", y = "Matches")

dkt <- d[Owner == "KT"]
plotm3 <- ggplot(dkt, aes(x = Date, y = Matches, colors = Date)) +
  geom_line(color = "blue", size = 5) +
  labs(title = "Likes over Time (KT)", x = "Date", y = "Matches")

dlu <- d[Owner == "Luka"]
plotm4 <- ggplot(dlu, aes(x = Date, y = Matches, colors = Date)) +
  geom_line(color = "blue", size = 5) +
  labs(title = "Likes over Time (Luka)", x = "Date", y = "Matches")
...
stacked_match_plots <- plotm1 / plotm2 /plotm3 / plotm4 / plotm5
stacked_match_plots
```
...
```{r matches_regression_complex, include=TRUE, warning=FALSE, results='asis'}
...
```{r likes_regression, include=TRUE, warning=FALSE, results='asis'}
...
```{r likes_regression_complex, include=TRUE, warning=FALSE, results='asis'}
...
```{r HTE_preprocessing, include=TRUE, warning=FALSE}
...
# HTE by Owner
```{r HTE_owner_average, include=TRUE, warning=FALSE}
...
```{r HTE_owner_anova, include=TRUE, warning=FALSE}
...
```{r HTE_city_avg, include=TRUE, warning=FALSE}
...
```{r HTE_owner_t, include=TRUE, warning=FALSE}
# T-Test for Likes
t_test_likes_city <- t.test(treatment_effect_likes ~ interaction(City_Control, City_Treat), data = final_dataset)
t_test_likes_city

# T-Test for Matches
t_test_matches_city <- t.test(treatment_effect_matches ~ interaction(City_Control, City_Treat), data = final_dataset)
t_test_matches_city
...
```{r HTE_weeks_average, include=TRUE, warning=FALSE}
#HTE by weeks
...
```{r HTE_weeks_anova, include=TRUE, warning=FALSE}
# ANOVA for Likes
